...
class MinVarHedge:
    ...
    def hedge(self) -> None:
        ...
        tqdm progress bar for tracking. For each day:
            ...
            2. Compute the hedge polynomial_regression_delta for the current day using
               `_compute_delta`.
    ...
    def roll_out_cashflows(self) -> None:
        ...
        computes the cashflows generated by applying the terminal polynomial_regression_delta of the
        ...
        For each asset day:
            ...
            2. For each asset (POWER and COAL):
                - Retrieve the terminal polynomial_regression_delta of the BOM.
                - Retrieve the corresponding spot price on the asset day.
                - Update the cashflows for that day by multiplying the
                  terminal polynomial_regression_delta with the spot price.
        ...
        for asset_day in self._asset_days:
            ...
            # Terminal polynomial_regression_delta of the current BOM
    ...
    def get_front_months_start_dates(self, hedge_date: pd.Timestamp) -> xr.DataArray:
        """
    ...
    def _compute_delta(
        ...
        """
        Compute hedge deltas for the given asset_day using polynomial_regression_delta regression
    ...
    def _hedge_front_month(
        ...
        """
        Compute hedge polynomial_regression_delta for a single delivery month using polynomial_regression_delta regression.
        ...
        -------
        np.ndarray
            Predicted hedge positions (polynomial_regression_delta) for all simulation paths.
        ...
        # Perform regression
        delta_position_results = self._delta_calculator.delta(
            fwds=fwds, y=y, beta=beta, efficiency=self._power_plant.efficiency
        )

        delta_positions = delta_position_results.delta_positions
        r2_score = delta_position_results.r_squared
        ...
        n_days_in_front_month: int = pd.Timestamp(delivery_start).days_in_month
