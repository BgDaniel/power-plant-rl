...
class MinVarHedge:
    def __init__(
        ...
        self._spots = {
            POWER: self._spots_power,
            COAL: self._spots_coal
        }
    ...
    def hedge(self):
        self.tqdm = tqdm(self._asset_days, total=len(self._asset_days) - 1,
                         desc="Determine minimal variance hedge", unit="day", )
        for simulation_day in self.tqdm:
            front_months_power = self._get_front_months(self._fwds_power, simulation_day)
    ...
    def _get_front_months(self, fwds: xr.DataArray, simulation_day: pd.Timestamp) -> xr.DataArray:
        """
    ...
    def _compute_delta(self, asset: str, simulation_day: pd.Timestamp, front_months_fwds: xr.DataArray) -> None:
        """
        ...
        for delivery_start in front_months_start_dates:
            ...
            self._deltas.loc[
                {SIMULATION_DAY: simulation_day, DELIVERY_START: delivery_start, ASSET: asset}
            ] = self._hedge_front_month(asset, delivery_start, front_months_fwds.to_pandas())


    def _hedge_front_month(self, asset: str, delivery_start: str, front_months_fwds: pd.DataFrame,
                           r2_threshold=0.0):

        days_in_front_month: pd.DatetimeIndex = pd.date_range(start=delivery_start, end=delivery_start + pd.offsets.MonthEnd(0))
        cashflows_from_asset = self._cashflows.loc[days_in_front_month].values

        alpha = cashflows_from_asset.sum(axis=0) # shape (n_sims,)

        beta = self._spots[asset].loc[days_in_front_month].values.sum(axis=0) # shape (n_sims,)

        y = alpha / beta # the sum of the spots over the delivery months are always non zero
